...
public class RobInteract : MonoBehaviour, IInteractible
{
    ...
    private void ShopCooldown() 
    {
        ...
        {
            if (minigame.GetComponent<Minigame_Cars>().passed && robbed == false) //minigame pass check
            {
                robButton.interactable = false;
                robbed = true;
            }
        ...
        {
            if (minigame.GetComponent<Minigame_Alarm>().passed && robbed == false) //minigame pass check
            {
                robButton.interactable = false;
                robbed = true;
            }
        ...
        {
            if (minigame.GetComponent<Minigame_Donut>().passed && robbed == false) //minigame pass check
            {
                robButton.interactable = false;
                robbed = true;
            }
    ...
    private void GetMoney() 
    {
        if (minigame.GetComponent<Minigame_KeyLocks>() != null)
        {
            if (minigame.GetComponent<Minigame_KeyLocks>().passed && moneyTaken > 0)
            {
        ...
        {
            if (minigame.GetComponent<Minigame_Diamonds>().passed && moneyTaken > 0)
            {
                for (int i = 0; i < xGridMoney; i++)
                {
                    for (int j = 0; j < zGridMoney; j++)
        ...
        {
            if (minigame.GetComponent<Minigame_Cars>().passed && moneyTaken > 0)
            {
                for (int i = 0; i < xGridMoney; i++)
                {
                    for (int j = 0; j < zGridMoney; j++)
                ...
                minigame.GetComponent<Minigame_Cars>().passed = false;
            }
        }
        else if (minigame.GetComponent<Minigame_Alarm>() != null)
        {
            if (minigame.GetComponent<Minigame_Alarm>().passed && moneyTaken > 0)
            {
                for (int i = 0; i < xGridMoney; i++)
                {
                    for (int j = 0; j < zGridMoney; j++)
        ...
        {
            if (minigame.GetComponent<Minigame_Donut>().passed && moneyTaken > 0)
            {
                for (int i = 0; i < xGridMoney; i++)
                {
                    for (int j = 0; j < zGridMoney; j++)
                ...
                minigame.GetComponent<Minigame_Donut>().passed = false;
            }
        }
    }
    #endregion
